...
export async function GET(req: NextRequest) {
  const tryEndpoint = async (endpoint: string) => {
    try {
      ...
      return { endpoint, status: res.status, error: await res.text().then(t => t.substring(0, 500)) };
    } catch (err) {
  ...
  const results = await Promise.all([
    tryEndpoint('/orders/1850?order_type=2'),
    tryEndpoint('/orders/1850/custom-fields?order_type=2'),
    tryEndpoint('/custom-field/order/1850?order_type=2'),
  ]);
  
  const dumpPath = path.join(process.cwd(), 'scratch', 'probe_final.json');
