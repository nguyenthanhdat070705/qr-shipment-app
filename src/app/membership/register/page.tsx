      ...
      <PageLayout title="Đăng ký HV" icon={<Crown size={18} className="text-yellow-500" />}>
        ...
        <div className="flex items-center justify-center min-h-[60vh] px-4">
          <div className="text-center max-w-md w-full">
            <div className="flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-full bg-emerald-100 mx-auto mb-4">
              <CheckCircle size={36} className="text-emerald-600 sm:hidden" />
              <CheckCircle size={40} className="text-emerald-600 hidden sm:block" />
            </div>
            <h2 className="text-lg sm:text-xl font-extrabold text-gray-900">Đăng ký thành công!</h2>
            <p className="text-xs sm:text-sm text-gray-500 mt-2">
              Hội viên <strong className="text-gray-800">{form.full_name}</strong> đã được thêm vào hệ thống
            </p>
            {memberCode && (
              <div className="mt-3 inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl">
                <Crown size={14} className="text-amber-600" />
                <span className="text-xs sm:text-sm font-bold text-amber-800 font-mono">{memberCode}</span>
              </div>
            )}
    ...
    <PageLayout title="Đăng ký HV mới" icon={<Crown size={18} className="text-yellow-500" />}>
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button onClick={() => router.back()} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500 transition-colors flex-shrink-0">
            <ArrowLeft size={20} />
          </button>
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl font-extrabold text-gray-900">Đăng Ký Hội Viên Mới</h1>
            <p className="text-xs sm:text-sm text-gray-500">Chương trình Hội Viên Trăm Tuổi — Phí 2,000,000đ / 10 năm</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              <div>
                ...
                <input className="w-full px-3 sm:px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all"
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
              {beneficiaries.map((b, i) => (
                <div key={i}>
                  {i > 0 && <hr className="mb-4 sm:mb-6 border-gray-100" />}
          ...
          {/* Bank Info */}
          <div className="bg-gradient-to-br from-[#1B2A4A] to-indigo-800 rounded-2xl p-4 sm:p-6 text-white">
            <p className="font-bold text-sm mb-3">🏦 Thông Tin Chuyển Khoản</p>
            <div className="grid grid-cols-[auto_1fr] gap-x-3 sm:gap-x-6 gap-y-1.5 sm:gap-y-2 text-xs sm:text-sm">
              <span className="text-white/60">Chủ TK:</span><span className="font-bold break-words">CÔNG TY CỔ PHẦN DỊCH VỤ TANG LỄ BLACKSTONES</span>
              <span className="text-white/60">Số TK:</span><span className="font-bold font-mono break-all">053 100 00 2 19 19</span>
              <span className="text-white/60">Ngân hàng:</span><span className="font-bold">Vietcombank</span>
              <span className="text-white/60">Nội dung:</span><span className="font-bold text-yellow-300 break-words">[Họ tên HV] chuyển tiền hội viên</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            {error && (
              <div className="px-3 sm:px-4 py-2.5 bg-red-50 border border-red-200 rounded-xl text-xs sm:text-sm font-semibold text-red-600">
                ⚠️ {error}
              </div>
            )}
            <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 sm:ml-auto">
              ...
              <button type="submit" disabled={loading}
                className="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-yellow-500 to-amber-600 text-white rounded-xl font-bold text-sm shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all disabled:opacity-50 disabled:cursor-not-allowed">
