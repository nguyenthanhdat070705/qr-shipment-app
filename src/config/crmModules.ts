...
export const CRM_MODULES: CrmModule[] = [
  ...
  // ───────────────────────────── 4. HỢP ĐỒNG BÁN ─────────────────────────────
  {
    ...
    columns: [
      ...
      // LƯU Ý: GetFly lưu KHÁCH HÀNG cá nhân ở nhóm vendor_* (ô "bên bán"),
      // còn Blackstones ở nhóm buyers_* (ô "bên mua"). Nên hiển thị vendor_* = Khách hàng.
      c('vendor_account_id', 'Khách hàng (id)'),
      c('vendor_account_name', 'Khách hàng', { table: true }),
      c('vendor_account_address', 'Địa chỉ khách hàng'),
      c('vendor_account_phone', 'SĐT khách hàng', { table: true }),
      c('vendor_contact_name', 'Người liên hệ'),
      c('vendor_bank_account_number', 'STK khách hàng'),
      c('buyers_account_id', 'Đơn vị (id)'),
      c('buyers_account_name', 'Đơn vị (bên mua)'),
      c('buyers_account_address', 'Địa chỉ đơn vị'),
      c('buyers_account_phone', 'SĐT đơn vị'),
      c('buyers_contact_name', 'Liên hệ đơn vị'),
      c('buyers_bank_account_number', 'STK đơn vị'),
      c('after_vat', 'Giá trị sau VAT (GetFly)', { type: 'money' }),
      c('discount', 'Chiết khấu (%)', { type: 'percent' }),
      c('discount_amount', 'Tiền chiết khấu', { type: 'money' }),
      ...
      c('total_payment', 'Giá trị HĐ', { type: 'money', table: true }),
