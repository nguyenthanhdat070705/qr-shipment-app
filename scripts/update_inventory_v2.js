...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('  CẬP NHẬT TỒN KHO DB V2 TỪ BẢNG MỚI NHẤT (TSV)');
  console.log('═══════════════════════════════════════════════════\n');
  ...
  // Xóa fact_inventory
  console.log("Xóa fact_inventory cũ trên V2...");
  ...
  for (const item of groupedData) {
    ...
    const khoId = khoMap[item.kho_ten] || khoMap['Kho ' + item.kho_ten];
    ...
    if (!khoId) {
      console.log(`Kho không hợp lệ trong V2: ${item.kho_ten}`);
      continue;
    }
  ...
  if (notFoundProducts.size > 0) {
    console.log("\n⚠️ Các mã không tìm thấy trong V2 dim_hom và sẽ bị bỏ qua (V2 có thể ít sản phẩm hơn V3):");
  ...
  console.log(`\n✅ Hoàn tất! Đã cập nhật ${insertRows.length} mục vào kho V2.`);
}
