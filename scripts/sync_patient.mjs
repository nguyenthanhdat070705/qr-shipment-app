// Wait much longer for PostgREST cache to naturally refresh (every ~5min on Supabase)
// then sync automatically
...
async function fetchAll(endpoint) {
  ...
  while (true) {
    ...
    all.push(...json.data);
    process.stdout.write(`\r  Trang ${page}: ${all.length}...`);
...
async function restUpsert(table, rows, col) {
  if (!rows.length) return { upserted: 0, errors: 0 };
  let upserted = 0, errors = 0;
  for (let i = 0; i < rows.length; i += 100) {
    const batch = rows.slice(i, i+100).filter(r => r[col] != null);
    if (!batch.length) continue;
    const r = await fetch(`${BASE}/rest/v1/${table}?on_conflict=${col}`, {
      method: 'POST',
      headers: { apikey: KEY, Authorization: `Bearer ${KEY}`, 'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates,return=minimal' },
      body: JSON.stringify(batch),
    });
    if (!r.ok) { const t = await r.text(); console.error(`\n  ❌ batch ${i}: ${t.slice(0,120)}`); errors += batch.length; }
    else { upserted += batch.length; process.stdout.write(`\r  Upserted: ${upserted}...`); }
  }
  console.log();
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log('║   1Office → Supabase Sync (chờ cache PostgREST)    ║');
  console.log('╚══════════════════════════════════════════════════════╝\n');
  console.log('⏳ PostgREST cache tự động refresh mỗi ~5 phút...\n');

  // Poll up to 15 minutes (180 x 5s)
  for (let i = 1; i <= 120; i++) {
    const [c, q] = await Promise.all([checkTable('sale_contracts'), checkTable('sale_quotations')]);
    const elapsed = Math.floor(i * 5 / 60);
    process.stdout.write(`\r  [${elapsed}m${i*5%60}s] contracts=${c?'✅':'❌'} quotations=${q?'✅':'❌'}`);
    if (c && q) {
      console.log('\n\n✅ Cache đã sẵn sàng! Bắt đầu sync...\n');
      break;
    }
    if (i === 120) {
      console.log('\n\n❌ Timeout sau 10 phút. Vui lòng chạy lại sau.');
      process.exit(1);
    }
    await sleep(5000);
  }
  ...
  if (rawC.length) {
    ...
    console.log(`  ✅ ${r.upserted} upserted, ${r.errors} lỗi`);
  ...
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log(`║  ${te===0?'✅ THÀNH CÔNG':'⚠️  XONG'} — ${dur}s`);
  console.log('╠══════════════════════════════════════════════════════╣');
  ...
  console.log(`║  TỔNG: ${tf} fetched, ${tu} upserted`);
  console.log('╚══════════════════════════════════════════════════════╝\n');
}
