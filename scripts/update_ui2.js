...
if (!content.startsWith("'use client';")) {
  console.log("File is corrupted before doing anything.");
}
...
// 4. Update the filter
content = content.replace(
  /\|\|\s*\(p\.NCC \|\| ''\)\.toLowerCase\(\)\.includes\(q\)/,
  `|| (p.Nguon_goc || '').toLowerCase().includes(q)`
);

// 5. Update Table Header
content = content.replace(
  /<th>Tính chất<\/th>\s*<th>ĐVT<\/th>\s*<th>NCC<\/th>/,
  `<th>Đặc điểm</th>\n                <th>ĐVT</th>\n                <th>Nguồn gốc</th>`
);

// 6. Update Table Body cells
content = content.replace(
  /<td className="pm-td-text">\{p\.tinh_chat \|\| '—'\}<\/td>\s*<td>\{p\.don_vi_tinh\}<\/td>\s*<td>\{p\.NCC \|\| '—'\}<\/td>/,
...
console.log('Successfully updated ProductsManageClient.tsx safely.');
